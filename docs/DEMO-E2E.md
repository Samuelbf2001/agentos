...
## 4. Bugs y hallazgos — estado tras B8 paso 1 (2026-08-28, commit `2b5aa4d`)

> Ciclo de corrección ejecutado: 10 de 12 corregidos (todos con test), 1 mitigado
> pendiente de verificación con run real, 2 diferidos a fase 2 con propuesta.
> Suite completa: **273 tests verdes**, `pnpm -r typecheck` limpio. Re-seed
> aplicado sobre la DB local y verificado (allowlists nuevas activas; prompt de
> Alex en versión nueva activa v3).

| # | Sev | Qué pasa | Repro | Estado (B8) |
|---|---|---|---|---|
| H1 | **ALTA** | El contexto de un run de chat no incluye el `project_id` real del thread: Alex inventa slugs (`assessment-acme`) y todo `tasks.create` falla con `not_found`. US-1 no funciona de una con tablero real. | Chat en un thread con `project_id` seteado → pedir crear tareas → audit del run muestra args con slug inventado. Corregir el ensamblado (`assemblePrompt`/volatile) para inyectar ids reales. | **CORREGIDO** (`2b5aa4d`): la capa context lleva el `project_id` real y la volatile de un run de chat lleva `thread_id`, proyecto activo del hilo e índice del tablero con los UUIDs reales de las tareas. Tests en `core/test/prompt.test.ts` y `api/test/channel-web.test.ts`. |
| H2 | **ALTA** | `board.get` con `project_id` inexistente devuelve tablero vacío OK en vez de `not_found` — alimenta la alucinación de H1 ("el tablero está vacío"). | `board.get {project_id:"lo-que-sea"}` → ok con 0 tarjetas. | **CORREGIDO** (`2b5aa4d`): valida el proyecto y lanza `not_found`. Test en `tools/test/context-tools.test.ts`. |
| H3 | **ALTA** | `knowledge.search` solo devuelve metadata (sin body) y varios agentes no tenían `knowledge.get` en su allowlist (seed) → no pueden leer las fuentes que deben citar (CA-12.2). | Con la allowlist sembrada de Sam: `knowledge.search` → ids; no hay forma de leer `body_md`. Workaround aplicado por API (allowlist de Sam); falta corregir `agents/*.md`/seed. | **CORREGIDO** (`2b5aa4d`): `search` devuelve `snippet` del body (FTS5 `snippet()`, equivalente Postgres `ts_headline` documentado) y los 7 `agents/*.md` ganan `knowledge.get`+`knowledge.list`. Re-seed aplicado y verificado en DB. Test del tool en `context-tools.test.ts`. |
| H4 | **ALTA** (fix 1 línea aplicado) | `delegate` y `ask_human` (tools de catálogo sin punto) no se namespaceaban a `mcp__agentos__*` en `claudeCodeAllowlist` → el CLI las negaba (fail-closed): ni delegar ni preguntar podía ningún agente `claude_code`. | Ver commit: `apps/api/src/domain-tools.ts` (condición del map). Añadir test de la allowlist con nombres sin punto. | **CORREGIDO** en B7b (`a44673c`). |
| H5 | MEDIA | B7a dejó los 7 agentes en runtime `claude_code` con modelos no-Anthropic en 3 de ellos (`gpt-5`, `kimi-...`, `MiniMax-M2`): el CLI no puede correrlos. Workaround por `PATCH /api/agents`; corregir seed/md o validar modelo↔runtime al guardar. | `GET /api/agents` sobre DB sembrada. | **CORREGIDO** (`2b5aa4d`) en el seed: el fallback baja también el modelo a `sonnet` cuando no es Anthropic, y lo restaura si aparece la credencial (hash efectivo incluye el modelo). La validación modelo↔runtime en `PATCH /api/agents` queda propuesta para fase 2. Test en `db/test/migration-seed.test.ts`. |
| H6 | MEDIA | `quinn_review_activity_types` sembrado solo con tipos técnicos: en un engagement de consultoría (entregables `report`/`process_map`) Quinn jamás arrancaría (US-10 muerto en el caso de uso principal). Config editada en caliente para la demo. | Tarea `report` a REVIEW con la config del seed → no hay run de Quinn. | **CORREGIDO** (`2b5aa4d`): el default incluye `report`, `process_map`, `roadmap`, `iso_gap`, `org_profile`, `leak_analysis`. Una DB con la config ya editada la conserva (no se pisa). Test funcional en `api/test/dispatcher.test.ts`. |
| H7 | BAJA | Humano no puede des-priorizar READY→BACKLOG (`invalid_transition`), pero CA-2.4 dice que el humano arrastra por la misma máquina. Decidir: o la UI lo esconde o la máquina lo permite a humanos. | `POST /tasks/:id/move {to:BACKLOG}` desde READY como persona. | **CORREGIDO** (`2b5aa4d`): decisión = la máquina lo permite SOLO a humanos (despriorizar es del humano). Tests en `core/test/state-machine.test.ts`. |
| H8 | MEDIA | Sam pobló `processes` sin `source_doc_ids` (y no usó `processes.link_source`): CA-12.5 parcial. Prompt o validación de la tool ("as_is sin fuentes → warning"). | Ver entidad `...35fadce7`. | **CORREGIDO** (`2b5aa4d`): causa raíz = el schema de `processes.upsert` no aceptaba `source_doc_ids` y Sam no tenía `processes.link_source` en su allowlist (su prompt ya lo exigía). Ahora el schema lo acepta, un `as_is` sin fuentes devuelve warning de provenance, y Sam gana `processes.get`+`processes.link_source`. Tests en `context-tools.test.ts`. |
| H9 | MEDIA | `ask_human` sin mover la tarjeta → el despachador re-despacha y el agente repite la pregunta (run y aprobación duplicados). Falta convención/enforcement: pregunta pendiente ⇒ BLOCKED. | Tarea READY sin insumo → 2 runs, 2 aprobaciones iguales (una quedó huérfana). | **CORREGIDO** (`2b5aa4d`), enforcement en plataforma: run que termina con aprobación pendiente sin mover la tarjeta → el despachador la fuerza a BLOCKED(approval); al decidir una pregunta/entregable, la respuesta se comenta en el timeline y la tarjeta vuelve a READY. Test del ciclo completo en `api/test/dispatcher.test.ts`. |
| H10 | **ALTA** (US-4) | "Esperando por ti" no lista tarjetas en REVIEW con `requires_approval` (mostró 0 con 3 en REVIEW); solo aprobaciones de tools/preguntas. El botón Aprobar vive solo en el diálogo de la tarjeta. | REVIEW con requires_approval → /waiting vacío. | **CORREGIDO** (`2b5aa4d`): nuevo `GET /api/waiting` (aprobaciones + TODAS las tareas en REVIEW con artefactos — REVIEW→DONE siempre es humano), evento `review.changed` al topic `approvals`, y la bandeja pinta el entregable con artefacto renderizado y Aprobar→DONE/Rechazar; el badge suma ambos. Tests en `api/test/waiting.test.ts` y `web/test` (store + smoke). |
| H11 | MEDIA | Alex evita `delegate`: para el envío redactó él y pidió aprobación en vez de crear la tarea hija a Sally (su propio prompt lo manda a delegar). Consecuencia: CA-6.2 (arista) sin ejercitar. Revisar prompt/función delegate (¿la conocía tras H4?). | Chat pidiendo un envío de email. | **MITIGADO** (`2b5aa4d`): causa probable era H4 (el CLI le negaba `delegate`), ya corregido; además el prompt de Alex ahora manda explícitamente delegar la ejecución al especialista (versión nueva activa tras re-seed). Verificación con run real DIFERIDA a la próxima demo (B8 no lanza runs LLM). |
| H12 | MEDIA | Alex no tiene `projects.create` en el catálogo/allowlist: con un workspace virgen CA-1.1 es imposible por chat (el humano debe crear el proyecto). Decidir si se le da la tool o si CA-1.1 asume proyecto pre-creado. | Chat pidiendo assessment sin proyecto existente. | **DIFERIDO a fase 2** (exige tool nueva, no parche): propuesta = añadir `projects.create` (+ resolución/creación de organización) al catálogo con flag `requires_approval` por política —crear un engagement es decisión de negocio— y sumarla a la allowlist de Alex. Mientras, CA-1.1 asume proyecto pre-creado por el humano (como en la demo). |
| Obs | — | Los agentes `claude_code` pueden usar tools built-in del CLI no listadas (Sally usó `Read` sobre el workspace). Aceptable para leer su workspace; revisar en B8 qué más queda permitido por defecto. | Spans del run `8cb04524`. | **DIFERIDO a fase 2** (política de built-ins, no parche): propuesta = frontmatter con allowlist explícita de built-ins por agente (Read/Write/Bash solo capa implementación) y `disallowedTools` por defecto para red (`WebSearch`, `WebFetch`) en `claude-code-options`, manteniendo fail-closed el catálogo de dominio. |
