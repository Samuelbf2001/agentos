...
> **Adenda 2026-09-05 — tareas usables (rama `feat/tareas-ui`)**: migración **0006 (SQLite) / 0002 (PG)** añade `task_labels` (unión tarea↔etiqueta, indexable en los dos motores; un array JSON en `tasks` obligaría a escanear en SQLite). La búsqueda de tareas cubre ficha y comentarios: `tasks_fts` (contenido externo sobre `tasks`) y `task_comments_fts` (índice propio alimentado por triggers desde `task_events.payload->>'body'`, porque el comentario no es una columna); en Postgres son columnas generadas `task_tsv`/`comment_tsv` con GIN y un plan B ILIKE. Los artefactos subidos desde la interfaz NO viven en la base: la fila guarda una ruta relativa a la raíz de artefactos (`AGENTOS_ARTIFACTS_DIR`, el workspace del proyecto o `data/artifacts`), nunca dentro del árbol versionado.
...
> **Adenda 2026-08-28 — Postgres HABILITADO como backend alternativo** (spec y detalle en `POSTGRES.md`). SQLite sigue siendo el default; `AGENTOS_DB_DRIVER=postgres` + `AGENTOS_PG_URL` activa el otro motor desde el entrypoint separado `@agentos/db/pg` (las 25 tablas, migraciones en `drizzle-pg/`, tsvector en vez de FTS5, y **pgvector** para la búsqueda semántica del Context Hub — el quinto disparador). Se respetan las convenciones de esta sección al pie de la letra, y por eso la traducción del esquema fue mecánica. Lo que NO es portable tal cual, dicho sin adornos: los repositorios SQLite son **síncronos** (better-sqlite3) y no existe driver PG síncrono, así que los de Postgres son **asíncronos** con la misma forma de API; los llamantes de `core`/`api`/`mcp-admin` necesitarían una pasada de `await` (mecánica, pendiente), y el motor de launch de Módulos de Fase (§13.3), atado a la transacción síncrona, no está portado. `docs/POSTGRES.md` §5 lo detalla junto con la mejora opcional `FOR UPDATE SKIP LOCKED` para el claim.
