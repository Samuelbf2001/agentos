...
> Síntesis del orquestador sobre el diseño de Opus (deep-reasoner), con **verificación fáctica 12/12 contra el código** (matriz de transiciones, dirección de dependencias, aritmética del seed, patrón de transacciones). La segunda opinión independiente de Codex quedó en vuelo por una limitación del runtime (tareas background inconsultables); si llega, se reconcilia aquí antes de cerrar el bloque afectado.
...
- **`module_launches`** — recibo INMUTABLE append-only: FK a la **fila de versión concreta** + `module_slug`/`module_version` desnormalizados + **`blueprint_snapshot` (copia literal)** + `blueprint_hash` — triple candado NM-3: editar el módulo después no puede tocar un proyecto disparado. Además: `org_id`, `project_id`, `inputs` (literales, campos `sensitive` redactados) + `inputs_digest` (sha256 sin redactar), `toggles`, `methodology_id`, `result` JSON (qué se materializó: tasks con key→taskId, gate, budget), `task_count`, `budget_phase_usd`/`budget_per_run_usd`, **`previous_launch_id`** (encadenado US-M3), `idempotency_key` (único), `actor`, `duration_ms`.
