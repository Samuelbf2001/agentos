...
describe("EventBus", () => {
  it("asigna seq monotónico por topic y lo persiste en la DB", async () => {
    const db = makeDb();
    ...
    for (const i of [1, 2, 3, 4, 5]) {
      seqs.push((await bus.publish(topic, textEvent("r1", `d${i}`))).seq);
    }
    expect(seqs).toEqual([1, 2, 3, 4, 5]);
    expect(await bus.lastSeq(topic)).toBe(5);

    // topics independientes: cada uno arranca en 1
    expect((await bus.publish(SWARM_TOPIC, textEvent("r1", "x"))).seq).toBe(1);
  });

  it("notifica a suscriptores del topic y a suscriptores globales", async () => {
    const db = makeDb();
    const bus = new EventBus(db);
  ...
  it("resume: cae a la DB cuando el hueco excede el ring buffer", async () => {
    ...
    expect((await coldBus.getSince(topic, 4)).map((e) => e.seq)).toEqual([5, 6, 7, 8, 9, 10]);
  });

  it("la DB es la fuente de verdad: dos instancias de bus comparten el stream", async () => {
    const db = makeDb();
    const topic = runTopic("r5");
    const busA = new EventBus(db);
    const busB = new EventBus(db);

    await busA.publish(topic, textEvent("r5", "a"));
    const fromB = await busB.publish(topic, textEvent("r5", "b"));
    expect(fromB.seq).toBe(2); // continúa la secuencia persistida, no la suya local
    expect(await busA.getSince(topic, 0)).toHaveLength(2);
  });

  it("rechaza topics inválidos y payloads que no cumplen el schema", async () => {
    const db = makeDb();
    const bus = new EventBus(db);
    await expect(bus.publish("cualquier-cosa", textEvent("r", "x"))).rejects.toThrow(/Topic inválido/);
    await expect(
      bus.publish(SWARM_TOPIC, { type: "TEXT_MESSAGE_CONTENT" } as unknown as AgUiEvent),
    ).rejects.toThrow();
  });

  it("guarda run_id para indexar el stream por run", async () => {
    const db = makeDb();
    const bus = new EventBus(db);
    const persisted = await bus.publish(runTopic("r6"), textEvent("r6", "x"));
