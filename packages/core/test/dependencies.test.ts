...
async function humanDone(db: AgentosDb, engine: BoardEngine, taskId: string, actor: string): Promise<void> {
  let task = (await getTask(db, taskId))!;
  if (task.status === "READY") {
    await engine.moveTask({ taskId, to: "IN_PROGRESS", expectedVersion: task.version, actor });
    task = (await getTask(db, taskId))!;
  }
  await attachArtifact(db, { taskId, kind: "document", title: "Evidencia de cierre" });
  await engine.moveTask({ taskId, to: "DONE", expectedVersion: task.version, actor });
}
...
describe("guard de dependencias en BACKLOG→READY", () => {
  async function withDependent(f: Fixture) {
    const dep = await seedTask(f); // BACKLOG
    const dependent = await seedTask(f);
    const updated = await updateTask(f.db, dependent.id, { dependsOn: [dep.id] }, dependent.version);
    return { dep, dependent: updated };
  }

  it("system con deps insatisfechas → dependency_not_satisfied con los ids bloqueantes", async () => {
    const f = await fixture();
    const { dep, dependent } = await withDependent(f);
    let err: unknown;
    try {
    ...
    expect((await getTask(f.db, dependent.id))!.status).toBe("BACKLOG");
  });

  it("agente (incluso el orquestador) con deps insatisfechas → rechazado", async () => {
    const f = await fixture();
    const { dependent } = await withDependent(f);
    await expect(
      f.engine.moveTask({
        taskId: dependent.id,
        to: "READY",
    ...
    ).rejects.toThrowError(/dependencias sin cerrar/);
  });

  it("humano puede forzar (override) y queda auditado como tal", async () => {
    const f = await fixture();
    const { dep, dependent } = await withDependent(f);
    ...
    const audits = await queryAudit(f.db, { action: "task.dependency_override", entityId: dependent.id });
    ...
    const moveEvent = (await listTaskEvents(f.db, dependent.id)).find((e) => e.kind === "moved")!;
    expect(moveEvent.payload?.["dependencyOverride"]).toBe(true);
    expect(moveEvent.payload?.["unsatisfiedDependsOn"]).toEqual([dep.id]);
  });

  it("con TODAS las deps en DONE, system promueve sin error (y sin audit de override)", async () => {
    const f = await fixture();
    const { dep, dependent } = await withDependent(f);
    ...
    expect((await dependencyState(f.db, dependent.id)).satisfied).toBe(true);
    const moved = await f.engine.moveTask({
      taskId: dependent.id,
      to: "READY",
      expectedVersion: dependent.version,
    ...
    expect(moved.status).toBe("READY");
    expect(await queryAudit(f.db, { action: "task.dependency_override" })).toHaveLength(0);
  });

  it("dependencyState: una dep inexistente cuenta como insatisfecha (fail-closed)", async () => {
    const f = await fixture();
    const t = await seedTask(f);
    await updateTask(f.db, t.id, { dependsOn: ["no-existe"] }, t.version);
    const state = await dependencyState(f.db, t.id);
...
describe("promoteUnblockedTasks — hook en la transición a DONE (CA-M2.3)", () => {
  it("DONE parcial no promueve; el DONE de la última dep promueve por la máquina real", async () => {
    const f = await fixture();
    const actor = `person:${f.person.id}`;
    const dep1 = await seedTask(f, { status: "READY" });
    const dep2 = await seedTask(f, { status: "READY" });
    const dependent = await seedTask(f);
    await updateTask(f.db, dependent.id, { dependsOn: [dep1.id, dep2.id] }, dependent.version);
    const solo = await seedTask(f); // BACKLOG sin deps: el sistema NO lo toca

    await humanDone(f.db, f.engine, dep1.id, actor);
    expect((await getTask(f.db, dependent.id))!.status).toBe("BACKLOG"); // dep2 sigue abierta

    await humanDone(f.db, f.engine, dep2.id, actor);
    const promoted = (await getTask(f.db, dependent.id))!;
    expect(promoted.status).toBe("READY");
    expect((await getTask(f.db, solo.id))!.status).toBe("BACKLOG");

    // La promoción pasó por la máquina: task_event 'moved' de actor system + evento de board.
    const moveEvent = (await listTaskEvents(f.db, dependent.id)).find(
  ...
  it("fail-soft: la dependiente sin DoD se queda en BACKLOG y no tumba el DONE", async () => {
    const f = await fixture();
    const actor = `person:${f.person.id}`;
    const dep = await seedTask(f, { status: "READY" });
    const sinDod = await seedTask(f, { definitionOfDone: null });
    await updateTask(f.db, sinDod.id, { dependsOn: [dep.id] }, sinDod.version);

    await humanDone(f.db, f.engine, dep.id, actor); // no lanza
    expect((await getTask(f.db, dep.id))!.status).toBe("DONE");
    expect((await getTask(f.db, sinDod.id))!.status).toBe("BACKLOG");
  });

  it("invocable directa: devuelve los ids promovidos", async () => {
    const f = await fixture();
    const dep = await seedTask(f);
    f.db.$client.prepare(`UPDATE tasks SET status = 'DONE', version = version + 1 WHERE id = ?`).run(dep.id);
    const dependent = await seedTask(f);
    await updateTask(f.db, dependent.id, { dependsOn: [dep.id] }, dependent.version);

    expect(await f.engine.promoteUnblockedTasks(f.project.id)).toEqual([dependent.id]);
    expect(await f.engine.promoteUnblockedTasks(f.project.id)).toEqual([]); // idempotente
  });
});
...
describe("cadena kickoff→entrevistas del launch real (consultoría)", () => {
  it("kickoff DONE promueve las 3 entrevistas; los mapas esperan a las entrevistas", async () => {
    ...
    const status = async (key: string) => (await getTask(db, byKey.get(key)!))!.status;
    expect(await status("kickoff")).toBe("READY");
    expect(await status("entrevista:direccion")).toBe("BACKLOG");

    await humanDone(db, engine, byKey.get("kickoff")!, "person:ernesto");

    for (const key of ["entrevista:direccion", "entrevista:operaciones", "entrevista:ventas"]) {
      expect(await status(key), key).toBe("READY");
    }
    ...
    for (const key of [
      ...
      expect(await status(key), key).toBe("BACKLOG");
    }

    // Cerrar las 3 entrevistas promueve los 2 mapas (deps: [entrevista]).
    for (const key of ["entrevista:direccion", "entrevista:operaciones", "entrevista:ventas"]) {
      await humanDone(db, engine, byKey.get(key)!, "person:ernesto");
    }
    expect(await status("mapa_proceso:produccion")).toBe("READY");
    expect(await status("mapa_proceso:ventas_facturacion")).toBe("READY");
    expect(await status("fugas")).toBe("BACKLOG"); // depende también de los mapas
  });
});
