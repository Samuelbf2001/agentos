...
describe("assemblePrompt — 3 capas", () => {
  it("stable contiene identidad + constitución con PROVENANCE y NO_ANSWER_CAME", async () => {
    const f = await fixture();
    await createPromptVersion(f.db, {
      agentId: f.sam.id,
      stable: "Eres Sam, consultor de diagnóstico de Sixteam.",
      context: null,
      volatileTpl: null,
    });
    const prompt = await assemblePrompt(f.db, { agent: "sam" });
  ...
  it("context contiene metodología activa, Context Hub y DoD; volatile la tarea y el timestamp", async () => {
    const f = await fixture();
    await upsertMethodology(f.db, {
      slug: "assessment-14d",
      version: 1,
    ...
    const task = await seedTask(f, { status: "IN_PROGRESS" });
    const now = Date.UTC(2026, 7, 27, 12, 0, 0);
    const prompt = await assemblePrompt(f.db, { agent: f.sam, project: f.project.id, task: task.id, now });
  ...
  it("sin metodología registrada lo dice explícitamente (no inventa)", async () => {
    const f = await fixture();
    const prompt = await assemblePrompt(f.db, { agent: "alex", project: f.project.id });
  ...
  it("run de chat: volatile incluye thread_id, project_id real e índice de tareas con sus UUIDs", async () => {
    const f = await fixture();
    const thread = await getOrCreateThread(f.db, {
      channel: "web",
    ...
    const prompt = await assemblePrompt(f.db, { agent: "alex", project: f.project.id, thread });
  ...
  it("run de chat sin proyecto activo: lo dice explícitamente en vez de dejar el hueco", async () => {
    const f = await fixture();
    const thread = await getOrCreateThread(f.db, {
      channel: "web",
    ...
    const prompt = await assemblePrompt(f.db, { agent: "alex", thread });
    expect(prompt.volatile).toContain("sin proyecto activo");
  });

  it("run de tarea (task presente): el índice de tablero no se duplica, la tarea trae su id", async () => {
    const f = await fixture();
    const task = await seedTask(f, { status: "IN_PROGRESS" });
    const prompt = await assemblePrompt(f.db, { agent: f.sam, project: f.project.id, task: task.id });
