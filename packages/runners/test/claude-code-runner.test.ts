...
describe("buildClaudeCodeOptions (higiene de entorno, sin lanzar CLI)", () => {
  it("claude_subscription: ELIMINA ANTHROPIC_API_KEY del env del hijo (no facturar API por accidente)", () => {
    const db = makeDb();
    const profile = makeProfile(db, { slug: "claude-sub", kind: "claude_subscription", apiKeyEnv: null });
  ...
  it("anthropic_api: inyecta ANTHROPIC_API_KEY desde process.env[api_key_env]", () => {
    const db = makeDb();
    const profile = makeProfile(db, { slug: "anthropic-api", kind: "anthropic_api", apiKeyEnv: "MI_KEY_ANTH" });
  ...
  it("anthropic_api sin la variable en el entorno → provider_not_configured", () => {
    const db = makeDb();
    const profile = makeProfile(db, { slug: "anthropic-api2", kind: "anthropic_api", apiKeyEnv: "NO_EXISTE" });
    try {
  ...
  it("kinds no-Anthropic → runner_unavailable (fail-closed)", () => {
    const db = makeDb();
    const profile = makeProfile(db); // openai_compatible
    expect(() =>
      buildClaudeCodeOptions({ input: baseInput(profile), ctx: makeCtx(), baseEnv: {} as NodeJS.ProcessEnv }),
    ).toThrow();
  });

  it("persona vía systemPrompt append, allowlist, maxTurns, resume, permissionMode fail-closed", () => {
    const db = makeDb();
    const profile = makeProfile(db, { slug: "claude-sub2", kind: "claude_subscription", apiKeyEnv: null });
...
describe("ClaudeCodeRunner.run (queryFn inyectado, sin CLI real)", () => {
  ...
  it("proyecta los mensajes del SDK a AG-UI, guarda session id y cierra el run con tokens/coste", async () => {
    const db = makeDb();
    const profile = makeProfile(db, { slug: "claude-sub3", kind: "claude_subscription", apiKeyEnv: null });
    ...
    expect(getClaudeSessionId(db, ctx.runId)).toBe("sess-1");
  });

  it("cierra tool_use huérfanos con NO_ANSWER_CAME", async () => {
    const db = makeDb();
    const profile = makeProfile(db, { slug: "claude-sub4", kind: "claude_subscription", apiKeyEnv: null });
  ...
  it("result de error del CLI → run failed + RUN_ERROR", async () => {
    const db = makeDb();
    const profile = makeProfile(db, { slug: "claude-sub5", kind: "claude_subscription", apiKeyEnv: null });
    const queryFn: ClaudeQueryFn = async function* () {
  ...
  it("cancel(): abort → proceso hijo muere → run cancelled", async () => {
    const db = makeDb();
    const profile = makeProfile(db, { slug: "claude-sub6", kind: "claude_subscription", apiKeyEnv: null });
    const queryFn: ClaudeQueryFn = async function* (params) {
    ...
    expect(getRun(db, ctx.runId)!.status).toBe("cancelled");
  });
  ...
  it.skip("smoke real: query() contra el CLI local (manual)", async () => {
    const db = makeDb();
    const profile = makeProfile(db, { slug: "claude-sub-real", kind: "claude_subscription", apiKeyEnv: null });
